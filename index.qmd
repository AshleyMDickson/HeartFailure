...
```{r}
generate_data <- function(n, n_preds = 10, intercept = -1.5, coef_value = 0.7) {
...
single_simulation <- function(sim_id, n_train, n_ext, n_boot) {
  ...
  model <- lrm(as.formula(model_fn), data = train_data, x = TRUE, y = TRUE)
  
  ext_cal_slope <- calc_cal_slope(model, ext_data)
...
run_simulation <- function(n_train, n_ext, n_sims = 50, n_boot = 200) {
  ...
  cat("Completed", nrow(all_results), "simulations in", 
      round(as.numeric(Sys.time() - start_time, units = "secs"), 1), "s\n\n")
...
gen_boxplot <- function(all_results) {
  ...
  ggplot(plot_data, aes(x = n_train, y = cal_slope, fill = validation_type)) +
    ...
    labs(title = "Calibration Slope by Sample Size: Bootstrap vs External Validation",
         subtitle = "CS of 1 = perfect calibration",
